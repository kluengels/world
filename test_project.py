...
from project import load_countries, use_joker, get_random, filter_countries, create_answer_options, check_answer
...
class TestCountries:
    ...
    def test_filter_countries(self):
        ...
        filtered = filter_countries(self.countries, "borders")
        print(len(filtered), len(self.countries))
        assert len(filtered) == len(self.countries) - 5

        # mode "capitals": 1 country without capital should be excluded
        filtered = filter_countries(self.countries, "capitals")
        print(len(filtered), len(self.countries))
    ...
    def test_create_answer_options(self):
        ### mode "flags":  
        options = create_answer_options(self.countries, self.indexes, self.right_index, "flags")
        ...
        ### mode "capitals"
        options = create_answer_options(self.countries, self.indexes, self.right_index, "capitals")
        # should return 4 answers
        assert len(options) == 4
        ...
        assert[o for o in options if o["right"] == True][0]["name"] == "Washington, D.C."

        ### mode "population"
        options = create_answer_options(self.countries, self.indexes, self.right_index, "population")
        # should return 4 answers
        assert len(options) == 4
        # only 1 answer should be right
        assert len([o for o in options if o["right"] == True]) == 1
        ...
        ### mode "area"
        options = create_answer_options(self.countries, self.indexes, self.right_index, "area")
        # should return 4 answers
        assert len(options) == 4
        # "United States", Mozambique, Brazil, Albania should be in options
        countries = ["United States", "Mozambique", "Brazil", "Albania"]
